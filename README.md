...
**Кэш и обновления:** интерфейс и скрипты отдаются **сначала из сети** (сервис-воркер обновляет кэш в фоне), чтобы после деплоя не «залипала» старая вёрстка и логика. При смене версии сервис-воркера старый кэш удаляется при активации, страница **один раз перезагружается** под новый контроллер — офлайн по-прежнему работает за счёт актуального кэша. Версия кэша и прекэш генерируются автоматически (`npm run build:sw`, входит в сборку). `localStorage` хранит только данные партии и настройки музыки, а не разметку — если после обновления сайта всё ещё странное поведение, в DevTools → Application: **Unregister** service worker и **Clear site data**, затем обновите страницу.
